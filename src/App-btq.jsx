...
// ---------------------------------------------------------------------------
// Range comparison chart (inlined for build stability)
// ---------------------------------------------------------------------------

const CHART_MAX_RANGE = 200;

const CHART_BAND_CLASSES = {
  PB: "bg-destructive",
  Close: "bg-accent",
  Medium: "bg-secondary",
  Long: "bg-primary",
  Extreme: "bg-foreground/50"
};

const CHART_BAND_LABELS = {
  PB: "Point Blank",
  Close: "Close",
  Medium: "Medium",
  Long: "Long",
...
const WeaponRangeBar = ({ name, ranges }) => {
  const maxRange = ranges?.Extreme?.[1] ?? 0;
  ...
  return (
    <div className="flex items-center gap-2 py-1">
      ...
      <div className="flex-1 h-5 bg-muted rounded-sm overflow-hidden relative">
        {Object.keys(ranges).map((band) => {
          const [start, end] = ranges[band];
          const width = ((end - start) / CHART_MAX_RANGE) * 100;
          const left = (start / CHART_MAX_RANGE) * 100;

          return (
            <Tooltip key={band}>
              <TooltipTrigger asChild>
                <div
                  className={`absolute h-full ${CHART_BAND_CLASSES[band]} opacity-70 hover:opacity-100 transition-opacity cursor-pointer`}
                  style={{
                    left: `${left}%`,
                    width: `${width}%`
                  }}
              ...
              <TooltipContent>
                <p className="font-semibold">{CHART_BAND_LABELS[band]}</p>
                <p className="text-xs">
                  {start} - {end} hex
                </p>
              </TooltipContent>
      ...
      <div className="w-12 text-xs text-muted-foreground text-right shrink-0">{maxRange}</div>
    </div>
  );
};

const RangeComparisonChart = () => {
  const [category, setCategory] = useState("all");

  const longGuns = Object.entries(RANGE_BANDS).filter(
    ([name]) => name.includes("Long") || name.includes("Swivel")
  );
  const carronades = Object.entries(RANGE_BANDS).filter(([name]) => name.includes("Carronade"));

  const displayedWeapons =
    category === "long" ? longGuns : category === "carronade" ? carronades : Object.entries(RANGE_BANDS);

  const sortedWeapons = [...displayedWeapons].sort((a, b) => b[1].Extreme[1] - a[1].Extreme[1]);

  return (
    <TooltipProvider>
      <Card className="w-full">
        <CardHeader className="pb-2">
          <CardTitle className="text-lg">Weapon Range Comparison</CardTitle>
          <CardDescription>Visual comparison of effective ranges for all weapon types (in hexes)</CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs value={category} onValueChange={(v) => setCategory(v)} className="mb-4">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="all">All Weapons</TabsTrigger>
              <TabsTrigger value="long">Long Guns</TabsTrigger>
              <TabsTrigger value="carronade">Carronades</TabsTrigger>
            </TabsList>
          </Tabs>

          <div className="flex flex-wrap gap-3 mb-4 text-xs">
            {Object.keys(CHART_BAND_CLASSES).map((band) => (
              <div key={band} className="flex items-center gap-1">
                <div className={`w-3 h-3 rounded-sm ${CHART_BAND_CLASSES[band]}`} />
                <span className="text-muted-foreground">{CHART_BAND_LABELS[band]}</span>
              </div>
            ))}
          </div>

          <div className="flex items-center gap-2 mb-2 text-xs text-muted-foreground">
            <div className="w-28 shrink-0">Weapon</div>
            ...
            <div className="w-12 text-right shrink-0">Max</div>
          </div>

          <div className="space-y-0.5">
            {sortedWeapons.map(([name, ranges]) => (
              <WeaponRangeBar key={name} name={name} ranges={ranges} />
            ))}
          </div>

          <div className="mt-4 p-3 bg-muted/50 rounded-md text-xs text-muted-foreground">
            <strong>Historical Note:</strong> Range bands reflect actual barrel lengths from the period. Lighter 18#
            and 24# Long guns often had longer barrels than heavier calibers, giving them greater range despite lower
            weight.
          </div>
        </CardContent>
      </Card>
